...
def _assert_safe_email(
    ...
    if scan.tags & {
        ...
        raise ValueError(
            "No forms or input fields in email."
        )
    ...
    for phrase in _CRED_ASK:
        ...
        if phrase in body:
            ...
            raise ValueError(
                f"Email asks the recipient for credentials: {phrase!r}"
            )
    ...
    for url in scan.urls:
        ...
        if not low.startswith("https://"):
            ...
            raise ValueError(
                f"Email links/assets must be absolute https: {url!r}"
            )
    ...
    for href, text in scan.anchors:
        ...
        for match in _HOSTISH.finditer(text):
            ...
            if not _same_site(
                ...
                raise ValueError(
                    f"Anchor text {match.group(1)!r} "
                    f"does not match real host {real!r}"
                )
...
def _wrap_email(
    ...
    return (
        ...
        'color:#f2f0eb">'
        f'{body_html}'
        '<p style="font-size:11px;color:#7c7e89;'
...
async def send_invite_email(
    ...
    body = (
        ...
        'to join the Signature Studio team as a '
...
async def reset_password(
    ...
    record = await db.password_reset_tokens.find_one(
        {
            "token": hash_token(payload.token),
            "used": False,
            "expires_at": {
                "$gt": now
            }
...
async def accept_invite(
    ...
    record = await db.invitations.find_one(
        {
            "token": hash_token(payload.token),
            "used": False,
            "expires_at": {
...
# ============================================================
# AUTH - READ INVITATION
# ============================================================

@api.get(
    "/auth/invitation/{invite_token}"
)
...
async def create_item(
    ...
    if payload.media_type not in {
        ...
        raise HTTPException(
            status_code=400,
            detail="Choose a reel or design."
        )

    if not (
        payload.media_url
        or payload.media_data
    ):

        raise HTTPException(
            status_code=400,
            detail="Add media before uploading."
        )
    ...
    category_exists = await db.categories.find_one(
        {
            "name": category
        }
    )
    ...
    result = await db.portfolio.insert_one(
        doc
    )
...
@api.patch(
    "/admin/portfolio/{item_id}/featured"
)
...
async def update_category(
    ...
    old = await db.categories.find_one(
        {
            "_id": object_id
        }
    )
    ...
    if payload.name is not None:
        ...
        if (
            new_name != old["name"]
            and await db.categories.find_one(
                {
                    "name": new_name
                }
            )
...
async def delete_category(
    ...
    if await db.portfolio.count_documents(
        {
            "category": doc["name"]
        },
        limit=1
        ...
        raise HTTPException(
            status_code=400,
            detail=(
                "Remove or move all works in this "
...
async def invite_member(
    ...
    if role not in ALL_ROLES:
        ...
        raise HTTPException(
            status_code=400,
            detail="Role must be OWNER or MANAGER"
        )
    ...
    try:
        ...
        logger.exception(
            "Invite email delivery failed for %s",
            email
        )
...
async def update_team_member(
    ...
    if payload.role is not None:
        ...
        if (
            ...
            raise HTTPException(
                status_code=400,
                detail="You cannot demote your own owner account."
            )
