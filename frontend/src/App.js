...
const CLOUDINARY_CLOUD_NAME = "dv6ab0bds";

const CLOUDINARY_UPLOAD_PRESET = "signature_studio_reels";
...
function PortfolioForm({ item, categories, close, saved }) {
  ...
  const onFile = (event) => {
    ...
    if (selected.size > 100 * 1024 * 1024) {
      ...
      setError("Choose a file under 100 MB.");
    ...
    try {
      ...
      if (file) {
        ...
        try {
          ...
          cloudinary = await response.json();
        ...
        if (!response.ok || !cloudinary?.secure_url) {

          throw new Error(cloudinary?.error?.message || "Cloudinary upload failed.");

        }

 

        finalUrl = cloudinary.secure_url;
      ...
      if (item.id) await client.put(`/admin/portfolio/${item.id}`, payload);

      else await client.post("/admin/portfolio", payload);
  ...
  return (
    ...
    <div className="modal-backdrop" onClick={close}>
      ...
      <form className="portfolio-form" onSubmit={submit} onClick={(event) => event.stopPropagation()} data-testid="portfolio-form">
        ...
        <button className="primary-button" type="submit" disabled={uploading} data-testid="save-portfolio-button">

          {uploading ? "UPLOADING..." : "SAVE TO LIVE PORTFOLIO"} <ArrowUpRight size={14} />
